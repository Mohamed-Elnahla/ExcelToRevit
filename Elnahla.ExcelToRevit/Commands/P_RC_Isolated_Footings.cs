...
internal class P_RC_Isolated_Footings : IExternalCommand
{
    public Result Execute(ExternalCommandData commandData, ref string message, ElementSet elements)
    {
        try
        {
            ...
            using (var ExcelPack = new ExcelPackage(new FileInfo(Excelfile)))
            {
                ...
                using (var PlacingRC_Footings = new Transaction(doc, "Placing RC Footings"))
                {
                    //Working on all Data in first 5 columns in first sheet
                    for (var row = 2; row < 9999; row++)
                    {
                        PlacingRC_Footings.Start();
                        ...
                        var Family_Symbol = new FilteredElementCollector(doc)
                            ...
                            .OfType<FamilySymbol>().Where(x =>
                                x.FamilyName == "RC-Iso_Footing" && x.Name == (string)workSheet.Cells[row, 5].Value)
                            .FirstOrDefault();
                        ...
                        var H_off = RC_Footing.LookupParameter("Height Offset From Level");
                        H_off.Set(0);
