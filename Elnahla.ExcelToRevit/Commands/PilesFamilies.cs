...
internal class PilesFamilies : IExternalCommand
{
    public Result Execute(ExternalCommandData commandData, ref string message, ElementSet elements)
    {
        try
        {
            ...
            //Family Path
            FileName = "C:/Program Files/Elnahla Revit Plugin/Families/Pile-Concrete.rfa";

            using (var trans = new Transaction(doc, "Import Piles Family"))
            {
                trans.Start();
                ...
                using (var newFamilyTypeTransaction = new Transaction(familyDoc, "Add Type to Family"))
                {
                    ...
                    for (var row = 2; row < 9999; row++)
                    {
                        ...
                        if (newFamilyType != null)
                        {
                            ...
                            var familyParam = familyManager.get_Parameter("Diameter");
                            if (null != familyParam)
                            {
                        ...
                        if (1 == changesMade) // set both paramaters?
                    ...
                    TaskDialog.Show("Revit", "New Piles Types Created Successfully");
                }
            }
