...
internal class F_RC_Isolated_Footings : IExternalCommand
{
    public Result Execute(ExternalCommandData commandData, ref string message, ElementSet elements)
    {
        try
        {
            ...
            //Family Path
            FileName = "C:/Program Files/Elnahla Revit Plugin/Families/RC-Iso_Footing.rfa";

            using (var trans = new Transaction(doc, "Import RC Footings Family"))
            {
                trans.Start();
                ...
                using (var newFamilyTypeTransaction = new Transaction(familyDoc, "Add Type to Family"))
                {
                    ...
                    for (var row = 2; row < 9999; row++)
                    {
                        ...
                        if (newFamilyType != null)
                        {
                            ...
                            var familyParam = familyManager.get_Parameter("Width");
                            if (null != familyParam)
                            {
                            ...
                            if (null != familyParam)
                            {
                                familyManager.Set(familyParam, MmToFoot((double)workSheet.Cells[row, 4].Value));
                                changesMade += 1;
                            }
                        }

                        if (3 == changesMade) // set both paramaters?
                    ...
                    TaskDialog.Show("Revit", "New RC Footings Types Created Successfully");
                }
            }
